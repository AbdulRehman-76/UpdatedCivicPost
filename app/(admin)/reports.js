...
import { getAllReports, getReportById, getDepartments } from '../../src/services/adminService';
import {
  normalizeStatus,
  getStatusStyle,
  getDeptIcon,
...
export default function AdminReportsScreen() {
  ...
  const [isSearchActive, setIsSearchActive] = useState(false); // when user has searched by ID
    ...
    return (
      ...
      >
        ...
        <View style={styles.reportFooter}>
          <Text style={styles.reportLocation} numberOfLines={1}>📍 {item.location || 'No location'}</Text>
          <Text style={styles.reportAssigned}>
            {item.assignedTo ? '👤 Assigned' : (item.category || 'Unassigned')}
          </Text>
        </View>
      </TouchableOpacity>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <Ionicons name={showDeptPicker ? 'chevron-up' : 'chevron-down'} size={16} color="#64748b" />
      </TouchableOpacity>
      ...
      {showDeptPicker && (
        <View style={styles.deptDropdown}>
          <TouchableOpacity
            style={[styles.deptDropdownItem, deptFilter === 'all' && styles.deptDropdownItemActive]}
            onPress={() => { setDeptFilter('all'); setShowDeptPicker(false); }}
          >
            <Text style={[styles.deptDropdownText, deptFilter === 'all' && styles.deptDropdownTextActive]}>All Departments</Text>
          </TouchableOpacity>
          {departments.map((d) => {
            ...
            return (
              <TouchableOpacity
                key={d.id}
                style={[styles.deptDropdownItem, active && styles.deptDropdownItemActive]}
                onPress={() => { setDeptFilter(d.name); setShowDeptPicker(false); }}
              >
                <Ionicons name={icon.iconName} size={16} color={icon.color} style={{ marginRight: 8 }} />
                <Text style={[styles.deptDropdownText, active && styles.deptDropdownTextActive]}>{d.name}</Text>
              </TouchableOpacity>
            );
          })}
        </View>
      ...
      <FlatList
        ...
        ListEmptyComponent={(
          <View style={styles.emptyState}>
            <Ionicons name="document-text-outline" size={52} color="#334155" />
...
const styles = StyleSheet.create({
  ...
  searchBtn: {
    width: 46, backgroundColor: '#8b5cf6', borderRadius: 12,
  ...
  deptDropdownItem:       { flexDirection: 'row', alignItems: 'center', padding: 12, borderBottomWidth: 1, borderBottomColor: '#334155' },
  deptDropdownItemActive: { backgroundColor: '#334155' },
  deptDropdownText:       { color: '#94a3b8', fontSize: 14 },
  ...
  reportIconBg:  { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  reportInfo:    { flex: 1, marginRight: 10 },
  reportTitle:   { fontSize: 15, fontWeight: '600', color: '#fff', marginBottom: 3 },
  reportId:      { fontSize: 11, color: '#475569', fontFamily: Platform.OS === 'ios' ? 'Courier' : 'monospace', marginBottom: 2 },
  reportDate:    { fontSize: 11, color: '#64748b' },
  statusBadge:   { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8 },
  statusText:    { fontSize: 11, fontWeight: '600' },
  reportFooter:  { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1, borderTopColor: '#334155', paddingTop: 10 },
