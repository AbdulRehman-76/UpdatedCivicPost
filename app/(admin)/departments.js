...
import { getDepartments, addDepartment, updateDepartment, deleteDepartment } from '../../src/services/adminService';
...
export default function DepartmentsScreen() {
  ...
  const openEditModal = (dept) => {
    ...
    setFormIcon(match || { label: dept.name, iconName: dept.iconName || 'help-circle-outline', color: dept.color || '#6366f1' });
  ...
  // ── Resolve icon for a department (handles legacy + new format) ──
  const resolveIcon = (dept) => {
    if (dept.iconName) return { iconName: dept.iconName, color: dept.color || '#6366f1' };
    const fallback = getDeptIcon(dept.name);
    return fallback;
  };
    ...
    return (
      ...
      >
        <View style={[styles.iconBg, { backgroundColor: icon.color + '22' }]}>
          <Ionicons name={icon.iconName} size={26} color={icon.color} />
        </View>
        <View style={styles.info}>
          <Text style={styles.name}>{item.name}</Text>
          <Text style={[styles.status, { color: isActive ? '#22c55e' : '#64748b' }]}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.emptyText}>No departments yet. Tap + to add one.</Text>}
        />
      )}
          ...
          <View style={styles.modalContent}>
            ...
            <View style={styles.modalHeaderRow}>
              <Text style={styles.modalTitle}>
                {modalMode === 'edit' ? 'Edit Department' : 'New Department'}
              </Text>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.iconScroll}>
                {ICON_OPTIONS.map((opt) => {
                  const active = formIcon.iconName === opt.iconName;
                  return (
                    <TouchableOpacity
                      key={opt.iconName}
                    ...
                    >
                      <View style={[
                        ...
                        active && { borderWidth: 2, borderColor: opt.color },
                      ]}>
                        <Ionicons name={opt.iconName} size={22} color={opt.color} />
                      </View>
                      <Text style={[styles.iconOptionLabel, active && { color: opt.color, fontWeight: '600' }]}>
                        {opt.label}
                      </Text>
                    </TouchableOpacity>
                  );
              ...
              <View style={styles.previewRow}>
                <View style={[styles.previewIconBg, { backgroundColor: formIcon.color + '22' }]}>
                  <Ionicons name={formIcon.iconName} size={28} color={formIcon.color} />
                </View>
                <Text style={styles.previewLabel}>{formName.trim() || 'Department Name'}</Text>
              </View>
                ...
                <>
                  ...
                  <View style={styles.readOnlyField}>
                    ...
                    <Text style={styles.readOnlyText}>
                      {editingDept?.createdAt ? formatDate(editingDept.createdAt) : 'Unknown'}
                    </Text>
                  </View>
                  ...
                  <View style={styles.toggleRow}>
                    <View>
                      ...
                      <Text style={styles.toggleDesc}>
                        {formIsActive ? 'Department is currently active' : 'Department is inactive'}
                      </Text>
                    </View>
                    <Switch
                      value={formIsActive}
              ...
              <View style={styles.modalButtons}>
                ...
                <TouchableOpacity
                  ...
                  onPress={modalMode === 'edit' ? handleUpdate : handleAdd}
                  disabled={submitting}
                >
                  {submitting ? (
                    ...
                    <Text style={styles.buttonText}>
                      {modalMode === 'edit' ? 'Update' : 'Add Department'}
                    </Text>
                  )}
                </TouchableOpacity>
              </View>
              ...
              {modalMode === 'edit' && (
                <TouchableOpacity style={styles.deleteButton} onPress={handleDelete} disabled={submitting}>
                  ...
                  <Text style={styles.deleteButtonText}>Delete Department</Text>
                </TouchableOpacity>
              )}
            </ScrollView>
          </View>
...
const styles = StyleSheet.create({
  ...
  list:      { padding: 16 },
  card:      { backgroundColor: '#1e293b', borderRadius: 14, padding: 16, marginBottom: 10, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#334155' },
  cardInactive: { opacity: 0.55 },
  iconBg:    { width: 50, height: 50, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  info:      { flex: 1 },
  ...
  input:         { backgroundColor: '#334155', borderRadius: 10, padding: 13, color: '#fff', fontSize: 16, marginBottom: 16 },

  // Icon scroll picker
  iconScroll:        { marginBottom: 16 },
  iconOption:        { alignItems: 'center', marginRight: 12, width: 68 },
  iconOptionCircle:  { width: 52, height: 52, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginBottom: 5 },
  iconOptionLabel:   { color: '#64748b', fontSize: 10, textAlign: 'center' },

  // Preview
  previewRow:     { flexDirection: 'row', alignItems: 'center', backgroundColor: '#334155', borderRadius: 12, padding: 12, marginBottom: 16 },
  previewIconBg:  { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  previewLabel:   { color: '#fff', fontSize: 16, fontWeight: '600' },

  // Read-only field (Created At)
  readOnlyField: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#334155', borderRadius: 10, padding: 12, marginBottom: 16 },
  readOnlyText:  { color: '#94a3b8', fontSize: 14 },

  // Toggle row (Active/Inactive)
  toggleRow:   { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#334155', borderRadius: 10, padding: 14, marginBottom: 20 },
  toggleLabel: { color: '#fff', fontSize: 15, fontWeight: '500', marginBottom: 2 },
  ...
  // Delete button
  deleteButton:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 20, padding: 14, borderRadius: 10, borderWidth: 1, borderColor: '#ef4444' },
