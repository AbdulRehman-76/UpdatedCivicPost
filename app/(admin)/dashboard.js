...
export default function AdminDashboardScreen() {
  ...
  const stats = useCallback(() => {
    ...
    let total = 0, pending = 0, inProgress = 0, resolved = 0, todayCount = 0;
    ...
    reports.forEach((r) => {
      ...
      if (formatDate(r.createdAt) === todayStr) todayCount++;
    });
  ...
  const renderRecentReport = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => router.push({ pathname: '/(admin)/report-detail', params: { id: item.id } })}
        activeOpacity={0.7}
      >
        <View style={styles.reportRow}>
          {/* Department Icon */}
          <View style={[styles.reportIconBg, { backgroundColor: deptIcon.color + '22' }]}>
            <Ionicons name={deptIcon.iconName} size={22} color={deptIcon.color} />
          </View>
        ...
        <View style={styles.reportFooter}>
          ...
          {item.priority && (
            <Text style={[styles.priorityBadge, priorityColor(item.priority)]}>
              {item.priority}
            </Text>
          )}
        </View>
      ...
      >
        ...
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.subtitle}>{t('welcomeAdmin') || 'Welcome, Admin'}</Text>
          </View>
        ...
        {/* ── By Department ── */}
        <Text style={styles.sectionTitle}>{t('byDepartment') || 'By Department'}</Text>
            ...
            departments.slice(0, 6).map((dept) => {
              ...
              return (
                <View key={dept.id} style={styles.deptRow}>
                  <View style={[styles.deptIconBg, { backgroundColor: icon.color + '22' }]}>
                    <Ionicons name={icon.iconName} size={18} color={icon.color} />
                  </View>
                  <View style={styles.deptInfo}>
                    ...
                    <View style={styles.progressBar}>
                      <View style={[styles.progressFill, { width: `${pct}%`, backgroundColor: icon.color }]} />
                    </View>
                  </View>
                </View>
              );
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>{t('recentReports') || 'Recent Reports'}</Text>
  ...
  return (
    <View style={styles.statCard}>
      <View style={styles.statHeader}>
        ...
        <View style={[styles.statIconBg, { backgroundColor: iconColor + '1A' }]}>
          <Ionicons name={icon} size={18} color={iconColor} />
        </View>
      </View>
      <Text style={[styles.statNumber, { color: iconColor }]}>{value}</Text>
      {sub?.trim() ? <Text style={[styles.statSub, { color: subColor }]}>{sub}</Text> : null}
    </View>
  );
}
...
function statusDisplayLabel(ns) {
  const m = { pending: 'Pending', assigned: 'Assigned', inProgress: 'In Progress', resolved: 'Resolved', closed: 'Closed' };
  return m[ns] || ns;
}
...
const styles = StyleSheet.create({
  ...
  // Stats Grid
  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 16, gap: 12, marginBottom: 28 },
  statCard: {
  ...
  // Sections
  sectionTitle: { fontSize: 17, fontWeight: 'bold', color: '#ffffff', paddingHorizontal: 20, marginBottom: 12 },
  sectionHeader: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
  ...
  deptCard: {
    ...
    padding: 16, marginBottom: 28, borderWidth: 1, borderColor: '#334155',
  },
  deptRow:   { flexDirection: 'row', alignItems: 'center', marginBottom: 14 },
  deptIconBg: { width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  deptInfo:  { flex: 1 },
  ...
  progressBar: { height: 5, backgroundColor: '#334155', borderRadius: 3, overflow: 'hidden' },
  ...
  reportIconBg: { width: 42, height: 42, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  reportInfo:   { flex: 1, marginRight: 10 },
  reportTitle:  { fontSize: 15, fontWeight: '600', color: '#ffffff', marginBottom: 3 },
  reportMeta:   { fontSize: 12, color: '#64748b' },
  statusBadge:  { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 8 },
  statusText:   { fontSize: 11, fontWeight: '600' },
  reportFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#334155' },
  reportId:     { fontSize: 11, color: '#475569', fontFamily: 'monospace' },
  priorityBadge:{ fontSize: 11, fontWeight: '600', paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
