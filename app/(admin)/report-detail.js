...
export default function AdminReportDetailScreen() {
  ...
  const loadDeptUsers = async (categoryName) => {
    try {
      const dept  = departments.find((d) => d.name === categoryName || d.id === categoryName);
  ...
  const handleAssignUser = async (user) => {
    setProcessing(true);
    try {
      await assignReport(report.id, user.id);
      setReport((prev) => ({
        ...prev,
        assignedTo:       user.id,
      ...
      Alert.alert('Success', `Report assigned to ${user.name}.`);
    } catch (e) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.statusBanner, { backgroundColor: statusStyle.bg }]}>
          <Ionicons
            name={STATUS_OPTIONS.find((s) => s.key === ns)?.icon || 'information-circle'}
            size={18}
        ...
        <View style={styles.card}>
          <View style={styles.titleRow}>
            <View style={[styles.titleIconBg, { backgroundColor: deptIcon.color + '22' }]}>
              <Ionicons name={deptIcon.iconName} size={24} color={deptIcon.color} />
            </View>
            <Text style={styles.reportTitle} numberOfLines={2}>{report.title}</Text>
          </View>

          <Text style={styles.reportDesc}>{report.description || 'No description provided.'}</Text>

          <View style={styles.metaGrid}>
            <View style={styles.metaItem}>
              <Ionicons name="location" size={14} color="#64748b" />
              <Text style={styles.metaText}>{report.location || 'No location'}</Text>
            </View>
            <View style={styles.metaItem}>
            ...
            <View style={styles.metaItem}>
              ...
              <Text style={styles.metaText}>Priority: {report.priority || 'N/A'}</Text>
            </View>
            <View style={styles.metaItem}>
              <Ionicons name="document-text" size={14} color="#64748b" />
              <Text style={[styles.metaText, styles.idText]}>ID: {report.id}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.citizenCard}>
          {citizenInfo ? (
              ...
              <View style={styles.citizenLeft}>
                <View style={styles.citizenAvatar}>
                  <Text style={styles.citizenAvatarLetter}>
                    {citizenInfo.name?.charAt(0)?.toUpperCase() || '?'}
                  </Text>
                </View>
                <View style={styles.citizenInfo}>
                  <Text style={styles.citizenName}>{citizenInfo.name || 'Unknown Citizen'}</Text>
                  <Text style={styles.citizenPhone}>{citizenInfo.phone || 'No phone'}</Text>
                  <Text style={styles.citizenId}>User ID: {citizenInfo.id}</Text>
                </View>
              </View>
            ...
            <View style={styles.citizenLeft}>
              <View style={[styles.citizenAvatar, { backgroundColor: '#475569' }]}>
                <Ionicons name="person-outline" size={22} color="#94a3b8" />
              </View>
              <View style={styles.citizenInfo}>
                <Text style={styles.citizenName}>Unknown Citizen</Text>
                <Text style={styles.citizenPhone}>
                  {report.userId ? `User ID: ${report.userId}` : 'No submitter info'}
                </Text>
              </View>
            </View>
          )}
        ...
        <TouchableOpacity style={styles.selector} onPress={() => setShowDeptModal(true)}>
          <View style={styles.selectorLeft}>
            <View style={[styles.selectorIconBg, { backgroundColor: deptIcon.color + '22' }]}>
              <Ionicons name={deptIcon.iconName} size={18} color={deptIcon.color} />
            </View>
            <Text style={styles.selectorText}>{report.category || 'Not assigned'}</Text>
          </View>
        ...
        <TouchableOpacity style={styles.selector} onPress={() => setShowUserModal(true)}>
          <View style={styles.selectorLeft}>
            <View style={styles.selectorIconBg}>
              <Ionicons name="person" size={18} color="#8b5cf6" />
            </View>
            <Text style={styles.selectorText}>
              {report.assignedUserName || (report.assignedTo ? `User ${report.assignedTo}` : 'Unassigned')}
            </Text>
          </View>
          <Ionicons name="chevron-forward" size={18} color="#64748b" />
        </TouchableOpacity>
        {deptUsers.length === 0 && (
          <Text style={styles.hintText}>No users in this department.</Text>
        )}
        ...
        <View style={styles.statusGrid}>
          {STATUS_OPTIONS.map((opt) => {
            ...
            return (
              <TouchableOpacity
                key={opt.key}
                style={[styles.statusBtn, active && { backgroundColor: sStyle.bg, borderColor: sStyle.text }]}
                onPress={() => handleUpdateStatus(opt.key)}
                disabled={processing || active}
              >
                <Ionicons name={opt.icon} size={16} color={active ? sStyle.text : '#64748b'} style={{ marginRight: 6 }} />
                <Text style={[styles.statusBtnText, active && { color: sStyle.text, fontWeight: '700' }]}>
                  {opt.label}
                </Text>
              </TouchableOpacity>
            );
          ...
          <>
            ...
            <View style={styles.timelineWrap}>
              {[...report.timeline].reverse().map((entry, i) => {
                const eStyle = getStatusStyle(normalizeStatus(entry.status));
                return (
                  <View key={i} style={styles.timelineEntry}>
                    <View style={styles.timelineDotWrap}>
                      <View style={[styles.timelineDot, { backgroundColor: eStyle.text }]} />
                      {i < report.timeline.length - 1 && (
                        <View style={[styles.timelineLine, { backgroundColor: eStyle.text + '33' }]} />
                      )}
                    </View>
            ...
            <ScrollView style={styles.modalList}>
              {departments.map((dept) => {
                ...
                return (
                  ...
                  >
                    <View style={[styles.modalItemIconBg, { backgroundColor: dIcon.color + '22' }]}>
                      <Ionicons name={dIcon.iconName} size={18} color={dIcon.color} />
                    </View>
                    <Text style={[styles.modalItemText, active && styles.modalItemTextActive]}>{dept.name}</Text>
                    {active && <Ionicons name="checkmark" size={18} color="#8b5cf6" />}
                  </TouchableOpacity>
                );
              })}
            </ScrollView>
            <TouchableOpacity style={styles.modalCloseBtn} onPress={() => setShowDeptModal(false)}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalList}>
                ...
                return (
                  ...
                  >
                    <View style={styles.modalUserAvatar}>
                      <Text style={styles.modalUserAvatarText}>{user.name?.charAt(0) || 'U'}</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.modalItemText, active && styles.modalItemTextActive]}>{user.name}</Text>
                      <Text style={styles.modalItemSub}>{user.phone || ''}</Text>
                    </View>
                    {active && <Ionicons name="checkmark" size={18} color="#8b5cf6" />}
                  </TouchableOpacity>
                );
              ...
              {deptUsers.length === 0 && (
                <Text style={styles.modalEmptyText}>No users in this department.</Text>
              )}
            </ScrollView>
...
const styles = StyleSheet.create({
  ...
  // Status banner
  statusBanner:     { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 10, marginBottom: 16 },
  statusBannerText: { fontWeight: '600', fontSize: 15 },

  // Info card
  card:         { backgroundColor: '#1e293b', borderRadius: 14, padding: 16, marginBottom: 20, borderWidth: 1, borderColor: '#334155' },
  titleRow:     { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12 },
  titleIconBg:  { width: 42, height: 42, borderRadius: 12, alignItems: 'center', justifyContent: 'center', marginRight: 12, flexShrink: 0 },
  reportTitle:  { flex: 1, fontSize: 20, fontWeight: 'bold', color: '#fff', lineHeight: 26 },
  reportDesc:   { color: '#cbd5e1', fontSize: 15, lineHeight: 23, marginBottom: 16 },
  metaGrid:     { gap: 8 },
  ...
  // Section labels
  sectionLabel: { fontSize: 13, fontWeight: '600', color: '#64748b', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8, marginTop: 4 },
  ...
  citizenAvatar:      { width: 48, height: 48, borderRadius: 24, backgroundColor: '#8b5cf6', alignItems: 'center', justifyContent: 'center', marginRight: 14 },
  citizenAvatarLetter:{ color: '#fff', fontSize: 20, fontWeight: 'bold' },
  citizenInfo:        { flex: 1 },
  citizenName:        { color: '#fff', fontSize: 16, fontWeight: '600', marginBottom: 2 },
  ...
  selectorIconBg:{ width: 36, height: 36, borderRadius: 10, alignItems: 'center', justifyContent: 'center', backgroundColor: '#334155', marginRight: 10 },
  ...
  statusBtn:     { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 9, borderRadius: 10, borderWidth: 1, borderColor: '#334155', backgroundColor: '#1e293b' },
  ...
  timelineNote:    { color: '#cbd5e1', fontSize: 14, fontWeight: '500', marginBottom: 2 },
  timelineDate:    { color: '#64748b', fontSize: 12 },

  // Modals
  modalOverlay:          { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'flex-end' },
  modalContent:          { backgroundColor: '#1e293b', borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, maxHeight: '75%' },
  modalTitle:            { color: '#fff', fontSize: 18, fontWeight: 'bold', textAlign: 'center', marginBottom: 16 },
  modalList:             { marginBottom: 12 },
  modalItem:             { flexDirection: 'row', alignItems: 'center', padding: 14, borderBottomWidth: 1, borderBottomColor: '#334155' },
  modalItemActive:       { backgroundColor: '#334155' },
  modalItemIconBg:       { width: 34, height: 34, borderRadius: 9, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  ...
  modalUserAvatar:       { width: 36, height: 36, borderRadius: 18, backgroundColor: '#334155', alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  modalUserAvatarText:   { color: '#fff', fontWeight: 'bold', fontSize: 16 },
  modalEmptyText:        { color: '#64748b', textAlign: 'center', padding: 24, fontSize: 14 },
  modalCloseBtn:         { padding: 14, backgroundColor: '#475569', borderRadius: 10, alignItems: 'center' },
